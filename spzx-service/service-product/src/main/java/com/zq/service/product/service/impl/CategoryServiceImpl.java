...
//        挑出一级分类，ParentId=0；
        List<Category> Onecollect = categoryList.stream().filter(it -> it.getParentId() == 0).toList();

      Onecollect.forEach( one->{
//        挑出二级分类，并将二级分类添加到以及分类中
                  List<Category> twocategoryList = categoryList.stream().filter(it -> it.getParentId().longValue() == one.getId()).toList();
          one.setChildren(twocategoryList);
//              跳出三级分类，并将三级分类添加到二级分类中
            twocategoryList.forEach(two ->{
                List<Category> categoryList3 = categoryList.stream().filter(it -> it.getParentId().longValue() == two.getId()).toList();
